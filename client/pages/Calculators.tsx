...
const ValueChip = ({ children }: { children: React.ReactNode }) => (
  <span className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-white/10 text-white/90 text-sm">
    {children}
  </span>
);
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {(["SIP","LUMPSUM","FD","RD"] as Tab[]).map(t => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={`px-4 py-1.5 rounded-full text-sm tracking-wide border ${tab===t?"bg-white/10 text-white border-white/20":"text-white/70 border-transparent hover:border-white/10"}`}
                >
                  {t}
                </button>
              ))}
              ...
              <div className="space-y-7">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-white/90">
                  <div className="rounded-lg bg-white/5 p-4">
                    <div className="text-white/60 text-xs uppercase">Invested amount</div>
                    <div className="text-lg">₹{formatINR(active.invested)}</div>
                  </div>
                  <div className="rounded-lg bg-white/5 p-4">
                    <div className="text-white/60 text-xs uppercase">Est. returns</div>
                    <div className="text-lg">₹{formatINR(active.returns)}</div>
                  </div>
                  <div className="rounded-lg bg-white/5 p-4">
                    <div className="text-white/60 text-xs uppercase">Total value</div>
                    <div className="text-lg">₹{formatINR(active.value)}</div>
                  </div>
                </div>
